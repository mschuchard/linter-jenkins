  ...
  provideLinter() {
    return {
      ...
      lint: async (textEditor) => {
        ...
        if (!(/Jenkinsfile/.exec(file)) && (textEditor.getGrammar().scopeName === 'source.groovy')) return [];

        // initialize vars and ignore stderr if requested
        const opts = { ignoreExitCode: true, throwOnStderr: !(atom.config.get('linter-jenkins.ignoreStderr')) };
        let cmd = '';
        let args = [];
        ...
        if (/^SSH/.exec(atom.config.get('linter-jenkins.lintMethod'))) {
        ...
        } else if (/^CLI/.exec(atom.config.get('linter-jenkins.lintMethod'))) { // cli methods
        ...
        } else if (/^CURL/.exec(atom.config.get('linter-jenkins.lintMethod'))) { // curl methods
          cmd = 'curl';
          ...
          if (atom.config.get('linter-jenkins.curl.useCrumb')) {
            ...
            if (atom.config.get('linter-jenkins.curl.authToken') === '')
              crumbRequest = this.helpers.exec(cmd, ['-L', '-s', `${atom.config.get('linter-jenkins.curl.httpURI')}/crumbIssuer/api/xml?xpath=concat(//crumbRequestField,":",//crumb)`]);
            else
              crumbRequest = this.helpers.exec(cmd, ['-L', '-s', '--user', atom.config.get('linter-jenkins.curl.authToken'), `${atom.config.get('linter-jenkins.curl.httpURI')}/crumbIssuer/api/xml?xpath=concat(//crumbRequestField,":",//crumb)`]);
        ...
        // lint
        return this.helpers.exec(cmd, args, opts).then(output => {
          const toReturn = [];
          ...
          output.split(/\r?\n/).forEach((line) => {
            ...
            if (matches != null) {
              toReturn.push({
                ...
                location: {
                  file,
                  position: [[Number.parseInt(matches[2], 10) - 1, Number.parseInt(matches[3], 10) - 1], [Number.parseInt(matches[2], 10) - 1, Number.parseInt(matches[3], 10)]],
                },
              });
            }
          });
