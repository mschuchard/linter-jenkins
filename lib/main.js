      ...
      lint: (activeEditor) => {
        ...
        if (/^SSH/.exec(atom.config.get('linter-jenkins.lintMethod'))) {
          ...
          if (atom.config.get('linter-jenkins.lintMethod') == 'SSH then CLI')
              args = args.concat(['-p', atom.config.get('linter-jenkins.ssh.port'), '-i', atom.config.get('linter-jenkins.ssh.key'), atom.config.get('linter-jenkins.ssh.userHost'), 'declarative-linter']);
          else  // ssh then full cli
              args = args.concat(['-p', atom.config.get('linter-jenkins.ssh.port'), '-i', atom.config.get('linter-jenkins.ssh.key'), atom.config.get('linter-jenkins.ssh.userHost'), `sudo java -jar ${atom.config.get('linter-jenkins.ssh.cliPath')} -s ${atom.config.get('linter-jenkins.ssh.httpURI')} declarative-linter`]);
        }
        // cli methods
        ...
        // curl methods
          ...
          if (atom.config.get('linter-jenkins.curl.useCrumb')) {
            ...
            args = ['-L', '-s', '-X', 'POST', '-H', crumb, '-F', `jenkinsfile=${content}`, `${atom.config.get('linter-jenkins.curl.httpURI')}/pipeline-model-converter/validate`];
          }
          else
            args = ['-L', '-s', '-X', 'POST', '-F', `jenkinsfile=${content}`, `${atom.config.get('linter-jenkins.curl.httpURI')}/pipeline-model-converter/validate`];
        }
        else {
          atom.notifications.addError(
            'Invalid linting method specified. Please consult the package settings GUI for the options.',
            {
