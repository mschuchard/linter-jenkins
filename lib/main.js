      ...
      lint: (activeEditor) => {
        ...
        // curl methods
          ...
          if (atom.config.get('linter-jenkins.curl.useCrumb')) {
            crumbRequest = helpers.exec(cmd, ['-s', `${atom.config.get('linter-jenkins.curl.httpURI')}/crumbIssuer/api/xml?xpath=concat(//crumbRequestField,":",//crumb)`]);
            crumbRequest.then(response => {
              localStorage.setItem('jenkinsCrumb', response);
            });
            crumb = localStorage.getItem('jenkinsCrumb');

            args = ['-s', '-X', 'POST', '-H', crumb, '-F', `jenkinsfile=${content}`, `${atom.config.get('linter-jenkins.curl.httpURI')}/pipeline-model-converter/validate`];
          }
          else
            args = ['-s', '-X', 'POST', '-F', `jenkinsfile=${content}`, `${atom.config.get('linter-jenkins.curl.httpURI')}/pipeline-model-converter/validate`];
        }
        else {
